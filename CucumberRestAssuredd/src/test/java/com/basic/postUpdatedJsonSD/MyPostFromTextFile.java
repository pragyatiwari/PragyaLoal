...
public class MyPostFromTextFile {
	...
	Response resp;
	 ...
	 public void user_hit_the_post_request_with_json_data(String username)
	 throws IOException {
	
	 // Read data from file
	 String data = "";
	 data = new
	 String(Files.readAllBytes(Paths.get("src/test/java/com/basic/postFromTextFileSD/user.json")));
	 ...
	 // Do POST request
	 resp =
	 RestAssured.given().accept(ContentType.JSON).relaxedHTTPSValidation().when().body(newJsonStr)
	 .post("https://reqres.in/api/users");
	 }

	@Given("^user hit the post request with \"([^\"]*)\" having \"([^\"]*)\" value$")
